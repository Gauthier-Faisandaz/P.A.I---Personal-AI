...
# leurs masques de decoupe depuis cadre.py, verifie que les SVG (cadres et
# icones meteo) sont utilisables par GTK, puis verifie que la geometrie
# recopiee dans eww.yuck est a jour, que chaque fenetre est symetrique
# haut/bas, que le coin de l'ecran est cache et que les masques sont bons.
#
...
#
# Icones meteo (hud/icones/*.svg) : dessinees a la main, pas generees. On
# les controle ici avec les cadres (memes pieges).
#
# Controles :
...
#     MEME chargeur SVG que GTK utilise. Un SVG mal forme ne fait pas
#     d'erreur dans eww : l'image est juste absente. Ici, on le voit. Pour
#     les cadres, on compare aussi la taille lue a celle de la fenetre.
#  3. "python3 cadre.py verifier" : geometrie recopiee dans eww.yuck (fenetres
#     et cases), symetrie haut/bas (bogue de picom 10.2, voir cadre.py), coin
#     de l'ecran cache par l'heure, masques presents, a la bonne taille et
#     symetriques.
#
...
erreurs=0
for f in $(for p in $PIECES; do echo "hud/$p.svg"; done) hud/icones/*.svg; do
  piece="$(basename "$f" .svg)"
  [ "$(dirname "$f")" = "hud/icones" ] && piece=""       # icone : pas de taille imposee
  ...
  # 2. Lecture GTK (+ taille attendue pour un cadre : celle de sa fenetre).
...
# 3. Geometrie recopiee dans eww.yuck, symetrie, coin, masques, cases.
